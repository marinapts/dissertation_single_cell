...
E13_hom_subset = check_cell_cycle(E13_hom_subset, m.s, m.g2m)
```

```{r warning=FALSE, message=FALSE}
E14_hom_subset = check_cell_cycle(E14_hom_subset, m.s, m.g2m)
```


```{r warning=FALSE, message=FALSE}
E13_het_subset = check_cell_cycle(E13_het_subset, m.s, m.g2m)
```


```{r warning=FALSE, message=FALSE}
E14_het_subset = check_cell_cycle(E14_het_subset, m.s, m.g2m)
```
...
SCTransform replaces the functions *NormalizeData*, *ScaleData*, and *FindVariableFeatures*. However, we do need to call `NormalizeData` before `CellCycleScoring`, since it expects as input the normalised raw count data.
...
```{r echo=FALSE}
E13_hom_normalised = SCTransform(E13_hom_subset, vars.to.regress = c('percent.mt', 'S.Score', 'G2M.Score'), verbose = FALSE)
E14_hom_normalised = SCTransform(E14_hom_subset, vars.to.regress = c('percent.mt', 'S.Score', 'G2M.Score'), verbose = FALSE)
E13_het_normalised = SCTransform(E13_het_subset, vars.to.regress = c('percent.mt', 'S.Score', 'G2M.Score'), verbose = FALSE)
E14_het_normalised = SCTransform(E14_het_subset, vars.to.regress = c('percent.mt', 'S.Score', 'G2M.Score'), verbose = FALSE)
...
run_plot_PCA = function(data) {
    ...
    feature_plot = FeaturePlot(object = data_pca, features = c('nCount_RNA', 'percent.mt'))
    phase_plot = DimPlot(data_pca, reduction = 'pca', group.by = 'Phase')
...
E13_hom_umap_subset = subset(E13_hom_umap, idents = c('9'), invert = TRUE)
...
E14_hom_umap_subset = subset(E14_hom_umap, idents = c('12'), invert = TRUE)
...
E13_het_umap_subset = subset(E13_het_umap, idents = c('9'), invert = TRUE)
...
E14_het_umap_subset = subset(E14_het_umap, idents = c('11'), invert = TRUE)
...
```{r}
cell_type_markers = c('Pax6', 'Eomes', 'Tbr1')
```


```{r}
FeaturePlot(E13_hom_umap_subset, reduction='umap', features=cell_type_markers)
```


```{r}
FeaturePlot(E14_hom_umap_subset, reduction='umap', features=cell_type_markers)
```


```{r}
FeaturePlot(E13_het_umap_subset, reduction='umap', features=cell_type_markers)
```


```{r}
FeaturePlot(E14_het_umap_subset, reduction='umap', features=cell_type_markers)
```
...
FeaturePlot(E13_hom_umap_subset, reduction='umap', features=ectopic_markers)
```


```{r fig.width=13, fig.height=7, message=FALSE, warning=FALSE}
FeaturePlot(E14_hom_umap_subset, reduction='umap', features=ectopic_markers)
```


```{r fig.width=13, fig.height=7, message=FALSE, warning=FALSE}
FeaturePlot(E13_het_umap_subset, reduction='umap', features=ectopic_markers)
```


```{r fig.width=13, fig.height=7, message=FALSE, warning=FALSE}
FeaturePlot(E14_het_umap_subset, reduction='umap', features=ectopic_markers)
```
